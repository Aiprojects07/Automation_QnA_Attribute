...
import requests
try:
    # OpenAI SDK for fallback provider
...
# ------------------------------
# Helicone (optional) integration helpers
# ------------------------------

def get_anthropic_url() -> str:
    """Return the direct Anthropic Messages endpoint. Helicone is disabled."""
    return ANTHROPIC_API_URL


def build_helicone_headers(properties: Dict[str, str] | None = None) -> Dict[str, str]:
    """Helicone disabled: return empty headers."""
    return {}
...
def call_claude_with_web_search(api_key: str, model: str, user_content: str, max_tokens: int = 30000, debug_basepath: str | None = None, system_text: str | None = None, request_headers_extra: Dict[str, str] | None = None) -> tuple[str, dict]:
    ...
    # Merge optional extra headers (e.g., Helicone)
...
# ------------------------------
# OpenAI fallback call w/ web search tool
# ------------------------------

def call_openai_with_web_search(openai_api_key: str, model: str, system_text: str, user_text: str, max_tokens: int = 12000, debug_basepath: str | None = None) -> tuple[str, dict]:
    """Call OpenAI Responses API with web_search tool as a fallback provider.
    We compose the same content (system + user) into a single input string.
    Returns (text, usage_info_like_dict).
    """
    logger = logging.getLogger(__name__)
    if OpenAI is None:
    ...
    # Compose a single input string to approximate system+user messages
    combined_input = f"System Instructions:\n{system_text}\n\nUser:\n{user_text}"

    try:
        logger.info(
            "LLM Request (OpenAI) -> model=%s, max_tokens=%s, tools=[web_search], msg_chars=%s",
            model, max_tokens, len(combined_input or "")
        )
        ...
        try:
            ...
            # Fallback parse: join any text outputs
            text = ""
            try:
        ...
        # Normalize usage to a plain dict so callers can use .get()
        def _usage_to_dict(u):
            try:
                if isinstance(u, dict):
                    return u
                # Some SDKs return a typed object (e.g., ResponseUsage)
                return {
        ...
        try:
            usage = _usage_to_dict(getattr(resp, "usage", {}))
        except Exception:
        ...
        if debug_basepath:
            try:
                os.makedirs(os.path.dirname(debug_basepath), exist_ok=True)
                with open(f"{debug_basepath}_response_openai_raw.json", "w", encoding="utf-8") as f_raw:
                    import json as _json
                    f_raw.write(_json.dumps(resp.to_dict() if hasattr(resp, "to_dict") else str(resp), ensure_ascii=False))
            except Exception:
                logger.debug("Failed to write OpenAI raw response for %s", debug_basepath)
...
def main() -> None:
    ...
    if anthropic_key:
        provider = "anthropic"
        logger.info("Provider selected: Anthropic (ANTHROPIC_API_KEY present)")
    elif openai_key:
        provider = "openai"
        logger.info("Provider selected: OpenAI (OPENAI_API_KEY present)")
        if OpenAI is None:
            logger.error("OpenAI SDK not installed. Please `pip install openai` or set ANTHROPIC_API_KEY.")
            sys.exit(2)
    else:
        logger.error("No LLM provider configured. Set either ANTHROPIC_API_KEY or OPENAI_API_KEY in .env")
        sys.exit(2)
        ...
        try:
            ...
            if provider == "anthropic":
                raw, usage_info = call_claude_with_web_search(
                    ...
                    request_headers_extra=None,
                )
            else:
                # OpenAI fallback path (Responses API with web_search tool)
                raw, usage_info = call_openai_with_web_search(
                    openai_key,
...
if __name__ == "__main__":
    ...
    # Working code automation working and used cache without helicone also using l* values